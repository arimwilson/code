...
public class ColorPickerPreference extends DialogPreference {
  ...
  // TODO(ariw): Get key from attrs!
  public ColorPickerPreference(Context ctx, AttributeSet attrs) {
    super(ctx, attrs);
    ctx_ = ctx;
    key_ = key;
    editor_ = ctx_.getSharedPreferences(SeismoWallpaper.PREFERENCES, 1).edit();
  }

  @Override
  protected View onCreateDialogView() {
    return new ColorPickerView(ctx_, new ColorPicked(), color_);
  }

  @Override
  protected void onDialogClosed(boolean positive_result) {
    if (positive_result) {
      editor_.putInt(key_, color_);
    }
  }
